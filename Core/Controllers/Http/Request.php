...
namespace Core\Controllers\Http;
...
class Request extends HttpBase {
	...
	private function attachHeaders() {
		header('HTTP/' . $this->httpVersion . ' ' . $this->statusCode. ' ' . Utils::$statusCodes[$this->statusCode]);
			...
			case "application/json":
				...
				if($body_params) {
					foreach($body_params as $param_name => $param_value) {
						$this->postParams[$param_name] = Utils::xssafe($param_value);
					}
				}
				break;
